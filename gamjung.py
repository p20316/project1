...
# =====================
st.title("🍀 공감형 감정 AI")
st.write("감정을 적고 엔터를 누르세요. `종료`라고 입력하면 분석 결과를 보여드려요.")

# 입력 폼 (전송 후 입력창 자동 초기화)
...
if submit_button and user_input:
    ...
    if text == "종료":
        ...
        if total > 0:
            ...
            st.write("이건 판단이 아니라, 네가 표현해 온 감정의 흐름이야. 이야기해 줘서 고마워.")
            ...
            st.warning("분석할 감정 데이터가 아직 없어요.")
            
    else:
        # 대화 기록 저장 및 응답 생성
        ai_res = empathic_response(text)
        st.session_state.chat_log.append(("나", text))
        st.session_state.chat_log.append(("AI", ai_res))

# 대화 로그 출력 (최신 메시지가 위로 오게 하려면 reversed 사용 가능)
